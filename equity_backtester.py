...
class FyersAuth:
        ...
        try:
            if os.path.exists(self.token_file):
                ...
                if token and time.time() + 600 < expiry:
                    logging.info(f"{Fore.GREEN}Found a valid saved access token.{Style.RESET_ALL}")
                    return token
                else:
                    logging.info(f"{Fore.YELLOW}Saved access token has expired or is invalid.{Style.RESET_ALL}")
        except Exception as e:
            logging.info(f"{Fore.RED}Error loading saved token: {e}{Style.RESET_ALL}")
        return None
    ...
    def save_token(self, token):
        ...
        try:
            ...
            logging.info(f"{Fore.GREEN}New access token saved successfully to {self.token_file}{Style.RESET_ALL}")
        except Exception as e:
            logging.info(f"{Fore.RED}Failed to save access token: {e}{Style.RESET_ALL}")
    ...
    def authenticate(self):
        ...
        try:
            ...
            # The input() function needs a print, so we can't replace all of them.
            # We will handle logging for these specific lines manually.
            print(f"\n{Fore.CYAN}Step 1: Please copy the URL below and paste it into your web browser:")
            logging.info(f"\nStep 1: Please copy the URL below and paste it into your web browser:")
            print(auth_url)
            logging.info(auth_url)
            print(f"\n{Fore.CYAN}Step 2: Log in to Fyers and authorize the application.")
            logging.info(f"\nStep 2: Log in to Fyers and authorize the application.")
            print(f"\n{Fore.CYAN}Step 3: After authorization, copy the 'auth_code' from the redirect URL.")
            logging.info(f"\nStep 3: After authorization, copy the 'auth_code' from the redirect URL.")

            auth_code = input(f"\n{Fore.YELLOW}Please enter the auth_code here: {Style.RESET_ALL}").strip()

            if not auth_code:
                logging.info(f"{Fore.RED}Authentication failed: No authorization code provided.{Style.RESET_ALL}")
                return None
            ...
            if response and response.get("access_token"):
                ...
                logging.info(f"{Fore.RED}Authentication failed: {response.get('message', 'Unknown error')}{Style.RESET_ALL}")
                return None
        except Exception as e:
            logging.info(f"{Fore.RED}An error occurred during authentication: {e}{Style.RESET_ALL}")
            return None
...
class FyersData:
    ...
    def get_bulk_intraday_data(self, symbols, date, resolution):
        ...
        for i, symbol in enumerate(symbols, 1):
            # Progress indicator will only be shown on the console, not written to the file for cleanliness.
...
class PreFilterEngine:
    ...
    def process_symbols(self, symbols: List[str]) -> (List[Dict], List[Dict]):
        ...
        for i, symbol in enumerate(symbols, 1):
            # Keep progress indicator on console only
            ...
            time.sleep(0.1)
        print() # Newline after progress indicator
        logging.info(f"{Fore.GREEN}Pre-filtering complete.{Style.RESET_ALL}")
...
class OrderManager:
    ...
    def generate_report(self):
        ...
        # The dataframe formatted as a string for logging
        logging.info(report_df.to_string())

        # Summary
        win_rate = (report_df['pnl_percent'] > 0).mean() * 100
        avg_pnl = report_df['pnl_percent'].mean()
        logging.info("\n--- Summary ---")
        logging.info(f"Total Trades: {len(report_df)}")
        logging.info(f"Win Rate: {win_rate:.2f}%")
        logging.info(f"Average P&L: {avg_pnl:.2f}%")
...
def main():
    ...
    if not access_token:
        logging.info(f"{Fore.RED}Authentication failed. Exiting.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        logging.info(f"{Fore.RED}Error: Symbol file '{cfg.SYMBOL_CSV}' not found. Exiting.{Style.RESET_ALL}")
        return

    filtered_stocks, _ = PreFilterEngine(fyers_data).process_symbols(all_symbols)

    if not filtered_stocks:
        logging.info(f"{Fore.YELLOW}No stocks passed the pre-filter. Exiting.{Style.RESET_ALL}")
        return
    ...
    if not all_5min_df.empty:
        ...
        logging.info(f"5-minute historical data saved to {hist_filename}")
    ...
    logging.info("Processing trades on 1-minute data...")
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        logging.error(traceback.format_exc()) # Log the full traceback to the file
